...
public class TryAgain {
	public static void main(String args[]){
		login("woniuyexing@163.com","cainiao97");
	}
	public static void login(String u, String p) {  
        ...
        try {  
            ...
            if (entity.replace("\"", "").indexOf("retcode=0") > -1) {  
                ...
                response = client.execute(getMethod);  
                entity = EntityUtils.toString(response.getEntity());
                getMethod = new HttpGet("http://weibo.com/p/1005053669102477/follow?relate=fans&page=20#place");  
                response = client.execute(getMethod);  
                entity = EntityUtils.toString(response.getEntity()); 
                File f = new File("C:\\working\\output.txt")  ;
    ...
    private static HashMap<String, String> preLogin(String unameBase64,DefaultHttpClient client)    
            throws IOException {    
        String url = "http://login.sina.com.cn/sso/login.php?entry=weibo&callback=sinaSSOController.preloginCallBack&su=&rsakt=mod&client=ssologin.js(v1.4.5)&_=" + "_=" + new Date().getTime();    
    ...
    private static String encodeAccount(String account) {  
        String userName = "";  
        try {  
        	//userName = Base64.encodeBase64Sting(URLEncoder.encode(account,"UTF-8").getBytes());
            userName = Base64.encodeToString(URLEncoder.encode(account,"UTF-8").getBytes());  
